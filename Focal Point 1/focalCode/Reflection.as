package focalCode {
	...
	public class Reflection {
		...
		public static var totalNumCollisions:uint = 0;
		public static var hitSoundChannel:SoundChannel;
		public static var hitVolume = 3.5;
		public static var hitSound:HitSound = new HitSound();
		private static var hitPlaying = false;
		private static var wtHitCount:int = 0;
		public static var quotaReached:Boolean = false;
		...
		public static function checkCollisions() {
			for (var i = 0; i < collisionList.length; i++) {
				...
				for (var j = 0; j < lightRayList.length; j++) {
					...
					if (collisionPoint && !tempLR.disabled) { //collided
						if (!quotaReached) totalNumCollisions++;
			...
			if (wtHitCount >= 1 && !hitPlaying) {
				hitSoundChannel = hitSound.play(0,int.MAX_VALUE);
				var hitVol:SoundTransform = new SoundTransform(hitVolume,0);
				hitSoundChannel.soundTransform = hitVol;
				hitPlaying = true;
			} else if (wtHitCount == 0 && hitPlaying) {
		...
		public static function addCollision(obj,lightRay:LightRay) {
			...
			var uniqueID = obj.name + lightRay.name;
			if (collidedList.indexOf(uniqueID) == -1) {
				if (obj is WaterTank) {
					obj.collided = true;
					wtHitCount++;
					Score.setMultiplier(wtHitCount);
				}
				collidedList.push(uniqueID);
			}
		}
		
		public static function removeCollision(obj,lightRay:LightRay) {
			var index = collidedList.indexOf(obj.name + lightRay.name);
			if (index != -1) {
				if (obj is Blimp) {
					obj.resetTint();
				}
		...
		private static function drawLight(sprite,obj,numLines,lrWidth,Y,lrHeight,checkNum,dontCheck=false) {
            ...
            g.clear();
			g.beginFill(0x000000,1), g.moveTo(0,startY);
		...
		public static function endDay() {
			...
			for (var i = 0; i < lightRayList.length; i++) {
				for (var j = 0; j < collisionList.length; j++) {
					removeCollision(collisionList[j],lightRayList[i]);
				}
			}
		}
		
		public static function endLevel() {
			totalNumCollisions = 0;
			quotaReached = false;
		}
		
		public static function reset() {
			collisionList = [];
			lightRayList = new Vector.<LightRay>();
			collidedList = new Vector.<Object>();
			totalNumCollisions = 0;
			wtHitCount = 0;
		}
	}
